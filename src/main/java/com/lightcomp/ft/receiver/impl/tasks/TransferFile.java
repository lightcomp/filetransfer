    ...
    private void writeChunk(InputStream is, WritableByteChannel wbch, long remaining) {
        ...
        while (true) {
            ...
            try {
                ...
                throw TransferExceptionBuilder.from("Failed to read file chunk input stream").addParam("fileId", fileId)
                        .setCause(e).build();
            }
            if (numR <= 0) {
                break;
        ...
        if (remaining > 0) {
            throw TransferExceptionBuilder.from("File chunk input stream ended prematurely").addParam("fileId", fileId)
