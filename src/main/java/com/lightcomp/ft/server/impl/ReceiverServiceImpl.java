...
import com.lightcomp.ft.server.impl.TransferImpl.ActivityStatus;
...
public class ReceiverServiceImpl implements ReceiverService, TransferProvider {
    ...
    private void run() {
        while (true) {
            ...
            synchronized (this) {
                ...
                processInactiveTransfers(timeoutedTransfers);
            }
    ...
    /**
     * Process all inactive transfers. Terminated inactive transfers can be removed
     * but inactive transfers with active state are only added to
     * timeoutedTransfers, because they must be canceled first. Caller must ensure
     * synchronization.
     */
    private void processInactiveTransfers(Collection<TransferImpl> timeoutedTransfers) {
